...
import { faBackward, faBackwardFast, faBackwardStep, faCookieBite, faCross, faForwardFast, faForwardStep, faPlay, faScrewdriverWrench, faWrench, faX, faXmark } from "@fortawesome/free-solid-svg-icons";
    ...
    <div className="relative w-full flex h-full container text-base text-primary">
      <div className="flex flex-row">
        ...
        <div className="flex gap-2 ml-auto">
          {simulation && simulation.cookieCutterSettings &&
          (<button onClick={(e) => {
            let target = e.target as HTMLButtonElement
            target.disabled = true
            let a = document.createElement("a")
            console.log(simulation.cookieCutterSettings)
            let blob = new Blob([simulation.cookieCutterSettings as string], {type: "application/json" })
            a.href = URL.createObjectURL(blob.slice(0, blob.size, blob.type))
            a.download = `${simulation.title.toLowerCase().replaceAll(" ", "_").toLocaleLowerCase()}_cookie_cutter_settings`
            a.click()
            target.disabled = false
          }} className="cursor-pointer group tooltip-container text-primary flex  gap-2">
            <span className="text-sm">Download Cookie Cutter Settings as .JSON</span>
            <FontAwesomeIcon icon={faCookieBite} className="text-lg text-gray-300 hover:text-primarybg hover:rotate-360 ease-in-out duration-300" ></FontAwesomeIcon>
          </button>)}
          <button onClick={() => setSimulationsMenuVisible(true)} className="cursor-pointer group tooltip-container text-primary flex gap-2">
            <span className="text-sm">Change Simulation</span>
            <FontAwesomeIcon icon={faScrewdriverWrench} className="text-lg text-gray-300 hover:text-primarybg hover:rotate-360 ease-in-out duration-300" ></FontAwesomeIcon>
          </button>
        </div>
      </div>
